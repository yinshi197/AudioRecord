...
void Encodec(AVFormatContext *ofmt_ctx, AVCodecContext *codec_ctx, AVFrame *frame)
{
    int ret;
...
int main(int argc, char **argv)
{
    ...
    const char *outfilename2 = "s16.ogg";
    FILE *out = nullptr;
	...
	*/
    ...
    //1.4 初始化输出文件
    ret = avformat_alloc_output_context2(&ofmt_ctx, nullptr, "ogg", outfilename2);
    if(ret < 0)
    {
        av_strerror(ret, error, sizeof(error) - 1);
    ...
    {
        ret = avio_open(&ofmt_ctx->pb, outfilename2, AVIO_FLAG_WRITE);
        if (ret < 0) 
        {
            av_strerror(ret, error, sizeof(error) - 1);
    ...
    while (av_read_frame(ifmt_ctx, pkt) == 0 && count < 2000)
    {
        ...
        if(av_audio_fifo_size(audio_fifo) >= frame->nb_samples)
        {
            ...
            Encodec(ofmt_ctx, codec_ctx, frame);
